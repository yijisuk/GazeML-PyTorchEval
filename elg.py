...
class ELG(torch.nn.Module):
    ...
    def __init__(self, first_layer_stride=1, num_rbs=1, num_hgs=4, num_modules=3,
        ...
        self.residualBlockPre1 = S.ResidualBlock(
            num_feature_maps, 2*num_feature_maps)
        self.residualBlockPre2 = S.ResidualBlock(
            2*num_feature_maps, num_feature_maps)

        self.hourglassBlock = nn.ModuleDict()
        self.hourglassAfter = nn.ModuleDict()
        for i in range(num_modules):
            self.hourglassBlock[f'{i}'] = S.HourglassBlock(
                num_hgs, num_feature_maps, num_rbs)
            self.hourglassAfter[f'{i}'] = S.HourglassAfter(
                num_feature_maps, num_landmarks, num_rbs)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_modules):
            ...
            x, h = self.hourglassAfter[f'{i}'](
                x_prev, x, do_merge=(i < (self.num_modules - 1)))
            x_prev = x
        heatmaps = h
