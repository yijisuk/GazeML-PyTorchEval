...
class HourglassBlock(nn.Module):
    ...
    def build_hg(self, steps_to_go, num_channels, num_rbs):
        ...
        for i in range(num_rbs):
            self.residualBlock[f'{steps_to_go}']['up'][f'{i}'] = ResidualBlock(
        ...
        for i in range(num_rbs):
            self.residualBlock[f'{steps_to_go}']['low'][f'{1}'][f'{i}'] = ResidualBlock(
                num_channels, num_channels)
        # Recursive
        if steps_to_go > 1:
            ...
            for i in range(num_rbs):
                self.residualBlock[f'{steps_to_go}']['low'][f'{2}'][f'{i}'] = ResidualBlock(
                    num_channels, num_channels)
        # RB3
        self.residualBlock[f'{steps_to_go}']['low'][f'{3}'] = nn.ModuleDict()
        for i in range(num_rbs):
            self.residualBlock[f'{steps_to_go}']['low'][f'{3}'][f'{i}'] = ResidualBlock(
                num_channels, num_channels)
    ...
    def forward_hg(self, x, steps_to_go, num_channels, num_rbs):
        ...
        # RB1
        for i in range(num_rbs):
            low1 = self.residualBlock[f'{steps_to_go}']['low'][f'{1}'][f'{i}'](
                low1)
        # Recursive
        if steps_to_go > 1:
            ...
            low2 = low1
            for i in range(num_rbs):
                low2 = self.residualBlock[f'{steps_to_go}']['low'][f'{2}'][f'{i}'](
                    low2)
        # RB3
        low3 = low2
        for i in range(num_rbs):
            low3 = self.residualBlock[f'{steps_to_go}']['low'][f'{3}'][f'{i}'](
                low3)
        # Upsampling
        low4 = F.interpolate(
            low3, size=up1.shape[2:], mode='bilinear', align_corners=True)
...
class HourglassAfter(nn.Module):
    def __init__(self, num_feature_maps, num_landmarks, num_rbs):
        ...
        self.residualBlock = nn.ModuleDict()
        for i in range(num_rbs):
            self.residualBlock[f'{i}'] = ResidualBlock(
...
class CalLandmarks(nn.Module):
    ...
    def forward(self, x):
        ...
        ref_xs = torch.reshape(ref_xs, (-1, h*w)).cpu()
        ref_ys = torch.reshape(ref_ys, (-1, h*w)).cpu()
